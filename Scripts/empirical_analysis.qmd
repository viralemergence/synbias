...
mutate(
    site = factor(site, levels = c("c2", "c3", "c6", "c7", "c8", "f1", "f2", "f3", "f4", "f5", "f1 (original)", "f5 (original)")),
    label_y = facilitative + 0.3
) |> 
    ggplot(aes(x = site)) +
    geom_col(
        aes(y = facilitative),
    ...
    geom_text(aes(y = label_y, label = paste0("n=", sample_size)), vjust = 0, size = 3) +
    labs(
        title = "Facilitative vs Competitive Interactions by Site",
        x = "Site",
        y = "Count (Facilitative above, Competitive below)"
    ) +
    scale_x_discrete(labels = c("c2", "c3", "c6", "c7", "c8", "f1", "f2", "f3", "f4", "f5", "f1\n(original)", "f5\n(original)")) +
    theme_minimal()
```
...
gp_model |>
    ...
    theme_minimal()
ggsave(here("Figures/supp_fig2_empirical_snail_fixed_effects.png"), width = 8, height = 4)
```

I'm frankly suspicious of these results as they stand; how could sample size not affect bias? The correlogram clearly shows a relationship. There is likely an issue with the model; consult with collaborators for advice.
...
There are a few facilitative pairs, which matches laboratory findings about secondary invaders in this system.
...
cooccurrence_analyses <- cooccurrence_matrices |>
    map(possibly(\(m) quiet(cooccur(m, thresh = T, spp_names = TRUE, prob = "comb")), otherwise = NULL))
```
...
daphnia |>
    ...
    mutate(
        idx = seq_len(n()),
        positive = map_int(idx, ~if_else(!is.null(cooccurrence_analyses[[.x]]), nrow(cooccurrence_analyses[[.x]]$results[cooccurrence_analyses[[.x]]$results$p_gt < 0.05, ]) %||% 0L, 0L)),
        negative = map_int(idx, ~if_else(!is.null(cooccurrence_analyses[[.x]]), nrow(cooccurrence_analyses[[.x]]$results[cooccurrence_analyses[[.x]]$results$p_lt < 0.05, ]) %||% 0L, 0L))
    ) |>
    select(-idx) |>
    pivot_longer(
...
This result is very different from the snail/trematode system. Very few associations are detected in Daphnia.
...
sig_pairs <- which(!map_lgl(cooccurrence_analyses, is.null)) |> 
    map_lgl(function(i) {
        rows <- cooccurrence_analyses[[i]]$results |> 
            filter(p_gt < 0.05 | p_lt < 0.05) |> 
            nrow()
        rows > 0
    }) |> which()
daphnia |>
    group_by(lake, `date collected`) |>
    group_keys() |> 
    dplyr::slice(which(!map_lgl(cooccurrence_analyses, is.null))[sig_pairs])
