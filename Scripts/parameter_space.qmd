...
I am exploring the parameter space in five ways. I am exploring the effects of interaction strength, ratio of competitive to facilitative interactions, number of strains, proportion of population sampled, baseline transmission rate, and presence/absence of priority effects.
...
if (simulation) {
  ...
  write_csv(results, "Data/simulation_round1.csv")
} else {
  results <- read_csv("Data/simulation_round1.csv", show_col_types = F)
}
head(results)
```
...
if (simulation) {
  ...
  results2 <- sample_data2 %>%
    mutate(richness_count = map_dbl(imperfect_samples, sum))
  write_csv(results2, "Data/simulation_round2.csv")
  ...
  results3 <- sample_data3 |> 
              mutate(richness_count = map_dbl(imperfect_samples, sum))
  write_csv(results3, "Data/simulation_round3.csv")
  toc()
} else {
  results2 <- read_csv("Data/simulation_round2.csv",
                       show_col_types = F)
  results3 <- read_csv("Data/simulation_round3.csv",
                       show_col_types = F)
}
```
...
if (simulation) {
  ...
  results4 <- sample_data3 |> 
              mutate(richness_count = map_dbl(imperfect_samples, sum))
  write_csv(results4, "Data/simulation_round4.csv")
  toc()
} else {
  results4 <- read_csv("Data/simulation_round4.csv",
                       show_col_types = F)
}
```
