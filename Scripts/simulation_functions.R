disease_sample <- function(matrix, prop, imperfect_assays = c(TRUE, FALSE)) {
  ...
  if (imperfect_assays) {
    ...
    for (c in 1:seq_len(perfect_sample)) {
      perfect_sample[, c][perfect_sample[, c] == 1] <- rbinom(
        strain_positives,
        1,
        1 - false_negative
      )
      perfect_sample[, c][perfect_sample[, c] == 0] <- rbinom(
        strain_negatives,
        1,
        false_positive
      )
...
df_to_inputs <- function(df) {
  ...
  if (df$priority_effects) {
    ...
    int_matrix[lower.tri(int_matrix)][comp_indices] <- runif(
      comp_int,
  ...
  diag(int_matrix) <- 1
  initial_pop <- c(
    rep(c(1, rep(0, df$strains)), df$strains),
