...
for i in 1:n_sims
    ...
    disease_mortality = rand(Truncated(Normal(scaled_plan[i,6], 1), 0, 1), n_strains)
    transmission = rand(Truncated(Normal(scaled_plan[i,7], 1), 0, 1), n_strains)
    latency = round.(Int, rand(Truncated(Normal(scaled_plan[i,8], 1), 1, 10), n_strains))
    recovery = rand(Truncated(Normal(scaled_plan[i,9], 1), 0, 1), n_strains)
    ...
    detections = virtual_ecologist_sample(
        ...
        proportion_sampled = scaled_plan[i, 4],
        false_positive_rate = 0.0,
        false_negative_rate = scaled_plan[i, 5]
    )
