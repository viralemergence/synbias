...
In the end, a very interesting result. The strong effects of timestep, number of strains, and proportion sampled were expected. It's interesting that latency is the most important epidemiological parameter in terms of the error rate on pathogen richness. Competition to facilitation ratio is more important than either recovery rate or transmission rate in determining the error rate.
...
train_matrix <- synbias_test[sample(1:250000, 2500)] |> # downsample or it'll take forever
...
library(patchwork)
partial.dependence.plot(colnames(train_matrix), shap_long, as.data.frame(train_matrix), c("#FF4D6FFF", "#579EA4FF", "#DF7713FF", "#F9C000FF", "#86AD34FF", "#5D7298FF", "#81B28DFF", "#7E1A2FFF", "#2D2651FF", "#C8350DFF", "#BD777AFF"))
```