...
Intriguingly, competition and facilitation scores are nearly insignificant in the frequency-dependent SI simulations. The most important parameters are number of strains, and timestep.
...
I think that epidemic duration explains a lot about the results I've seen here so far. The error rate depends on epidemic duration (how many timesteps the outbreaks are active) and epidemic duration is affected by the simulation parameters. I'm going to test this by measuring epidemic duration across simulations.
...
p1 = @ggplot(cumulative_detections_seir, aes(x = proportion_sampled, y = cumulative_detections, color = cf_cat, group = cf_cat)) +
