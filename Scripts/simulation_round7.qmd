...
matrices = create_interaction_matrix(input_df)
total_competition = Vector{Float64}()
total_facilitation = Vector{Float64}()
for m in matrices
    m0 = m .- 1
    negative_indices = m0 .< 0
    positive_indices = m0 .> 0
    c = sum(m0[negative_indices])
    f = sum(m0[positive_indices])
    push!(total_competition, c)
    push!(total_facilitation, f)
end
interaction_scores = DataFrame(sim=1:10000, total_competition=abs.(total_competition), total_facilitation=total_facilitation)
...
simulation_output_seir <- fread(here("Data/simulation_round7_output.csv"))[, sim := 1:10000] |>
	...
	_[, competition_score := total_competition / strains] |>
	_[, facilitation_score := total_facilitation / strains] |>
	_[, c("total_competition", "total_facilitation") := NULL]
synbias_split <- initial_split(simulation_output_seir[, -c("sim", "Detections", "cf_ratio", "interaction_strength")], strata = error_rate)
...
In the end, a very interesting result. The strong effects of timestep, number of strains, false negative rate, and proportion sampled were expected. It's interesting just how important the competition and facilitation scores are, more so than any of the epidemiological parameters.
...
shap_long_seir <- shap.prep(
  shap_contrib = shapley_data_seir$shap_score,
  X_train = train_matrix_seir
)
...
cumulative_detections_seir.cf_ratio_group = CategoricalArray(cumulative_detections_seir.cf_ratio_group,
    levels=["high competition", "neutral", "high facilitation"],
    ordered=true)

p1 =
    ggplot(cumulative_detections_seir, @aes(x = proportion_sampled, y = cumulative_detections, color = cf_ratio_group)) +
    geom_point(alpha=0.1) +
    geom_smooth(linewidth=20) +
    scale_y_continuous(name="Cumulative Parasite Species Detections") +
...
for i in 1:n_sims
    ...
    cumulative_detections_si[i] = sum(detection_by_strain)
end
...
cumulative_detections_si = @chain CSV.read("Data/simulation_round7_input.csv", DataFrame) begin
    ...
    @mutate(cf_ratio_group = case_when(
        cf_ratio <= 0.4 => "high competition",
        cf_ratio <= 0.6 => "neutral",
        cf_ratio > 0.6 => "high facilitation",
    ))
end

cumulative_detections_si.cf_ratio_group = CategoricalArray(cumulative_detections_si.cf_ratio_group,
    levels=["high competition", "neutral", "high facilitation"],
    ordered=true)

p2 =
    ggplot(cumulative_detections_si, @aes(x = proportion_sampled, y = cumulative_detections, color = cf_ratio_group)) +
    geom_point(alpha=0.1) +
...
handle_site_species_matrix <- function(file) {
    ...
    cooccurrence_analysis <- tryCatch(
        {
            quiet(cooccur(
                test_matrix,
                thresh = T,
    ...
    if (!is.na(cooccurrence_analysis)) {
        competitive_pairs <- cooccurrence_analysis |>
            _$results |>
...
pairs_df <- "Data/seir_site_species_matrices/" |>
    here() |>
    list.files(full.names = T) |>
    future_map(handle_site_species_matrix) |>
    discard(\(x) is.na(x)) |>
    list_rbind()
pairs_df
```

For the SEIR system, I can't detect interactions at any of the simulation/timestep matrices! This fits somewhat with the sparse findings from the *Daphnia* data, where there's a lot of turnover in the system and I had to control for the effects of time. Hard to detect interactions in that case. However, things may be different in the SI system.

```{julia simulate_communities_si}
input_df = CSV.read("Data/simulation_round7_input.csv", DataFrame)
matrices = create_interaction_matrix(input_df)
n_individuals = 1000
n_timesteps = 100
...
for (sim_idx, sim_data) in enumerate(results)
    for (timestep_idx, matrix) in enumerate(sim_data)
        int_matrix = Int.(Array(matrix))
        df = DataFrame(int_matrix, :auto)
        Arrow.write("Data/si_site_species_matrices/sim_$(sim_idx)_timestep_$(timestep_idx).arrow",
            df)
    end
end
```

Here I try to detect interacting pairs in the SI simulations.
...
test_matrix <- handle_site_species_matrix(
    "Data/si_site_species_matrices/sim_105_timestep_20.arrow"
)
pairs_df <- "Data/si_site_species_matrices/" |>
    here() |>
    list.files(full.names = T) |>
    future_map(handle_site_species_matrix) |>
    discard(\(x) is.na(x)) |>
...
This method can't detect interacting pairs in the SI simulations either!