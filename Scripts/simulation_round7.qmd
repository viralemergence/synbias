...
p1 = ggplot(cumulative_detections_seir, @aes(x = proportion_sampled, y = cumulative_detections, color = cf_ratio_group)) +
...
p2 = ggplot(cumulative_detections_si, @aes(x = proportion_sampled, y = cumulative_detections, color = cf_ratio_group)) +
    geom_point(alpha = 0.1) +
...
test_matrix <- read_feather("Data/seir_site_species_matrices/sim_100_timestep_10.arrow") |> as.matrix()
test_cooccur <- quiet(cooccur(test_matrix, thresh = T, spp_names = FALSE, prob = "comb"))
```