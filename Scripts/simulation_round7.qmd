...
final_wf_seir <- workflow() |>
    ...
    add_formula(error_rate ~ competition_score + facilitation_score + timestep + strains + proportion_sampled + false_negative + disease_mortality + transmission + latency + recovery) |>
...
final_wf_si <- workflow() |>
    ...
    add_formula(error_rate ~ competition_score + facilitation_score + timestep + strains + proportion_sampled + false_negative + disease_mortality + transmission) |>
...
formula_seir <- error_rate ~ competition_score + facilitation_score + timestep + strains + proportion_sampled + false_negative + disease_mortality + transmission + latency + recovery
train_matrix_seir <- synbias_test |>
    slice_sample(prop = 0.1) |>
    model.matrix(object = formula_seir, data = _) |>
    _[, -1] # remove intercept
var_names <- c("Competition Score", "Facilitation Score", "Timestep", "Strains", "Prop. Sampled", "False Negative", "Disease Mortality", "Transmission", "Latency", "Recovery")
...
cumulative_detections_seir = @chain cumulative_detections_seir begin
    ...
    @mutate(
        cf_cat = case_when(
            facilitation_score - competition_score <= -0.05 => "high competition",
            facilitation_score - competition_score <= 0.05 => "neutral", 
            facilitation_score - competition_score > 0.05 => "high facilitation"
        )
    )
    dropmissing()
end
...
p1 = ggplot(cumulative_detections_seir, @aes(x = proportion_sampled, y = cumulative_detections, color = cf_cat)) +
    ...
    geom_smooth(method = "loess", linewidth=3) +
